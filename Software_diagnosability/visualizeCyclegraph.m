...
% Initialisation of nodes and arcs
   ...
   % initial node with internal states and diagnosis
    ...
    % (RIMOSSA) la parte che chiude il ciclo! 🚀
    ...
    figure;
    hold on;
    h1 =plot(G, 'Layout', 'layered', 'NodeLabel',{}, 'EdgeLabel', {});
    ...
    h1.LineWidth=1;
    h1.EdgeColor='k';
    h1.ArrowSize = 0;  % Rende le frecce più visibili
    h2 = plot(G, 'Layout', 'layered', 'NodeFontSize',15,'NodeLabel', {}, 'EdgeFontSize', 15, 'EdgeLabel', labels);
    h2.Marker='none';
    h2.MarkerSize = 80;
    h2.NodeColor = 'white';
    h2.LineWidth = 1;
   ...
   % Settings for a horizontal layout
    h1.XData = linspace(1, numel(G.Nodes), numel(G.Nodes)); 
    h1.YData = ones(1, numel(G.Nodes)); 
   
    h2.XData = linspace(1, numel(G.Nodes.Name), numel(G.Nodes.Name)); 
    h2.YData = ones(1, numel(G.Nodes.Name)); 

    
    % Adding customised labels to nodes
    num_nodes = length(h2.XData);
    for i = 1:num_nodes
        x = h2.XData(i);
        y = h2.YData(i);
        
        % Customised knot label
        if isKey(node_labels, G.Nodes.Name{i})
            node_label = node_labels(G.Nodes.Name{i});
        else
            node_label = G.Nodes.Name{i};
        end
